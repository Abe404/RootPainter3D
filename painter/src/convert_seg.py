...
class ConvertThread(QtCore.QThread):
    ...
    def run(self):
        ...
        seg_fnames = [f for f in seg_fnames if im_utils.is_image(f)]
        for i, f in enumerate(seg_fnames):
            self.progress_change.emit(i+1, len(seg_fnames))
            if os.path.isfile(os.path.join(self.seg_dir, f)):
                seg = im_utils.load_image(os.path.join(self.seg_dir, f))
                converted_seg = self.conversion_function(seg)
                output_path= os.path.join(self.out_dir, f)
                img = nib.Nifti1Image(converted_seg, np.eye(4))
