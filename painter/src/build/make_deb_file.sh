...
cp src/build/RootPainter3D.desktop dist/ubuntu_setup/usr/share/applications


mkdir dist/ubuntu_setup/DEBIAN
cp -r src/build/DEBIAN_control dist/ubuntu_setup/DEBIAN/control
cp -r dist/RootPainter3D dist/ubuntu_setup/usr/local/bin/RootPainter3D
cp -r src/main/icons dist/ubuntu_setup/usr/local/bin/RootPainter3D/icons

dpkg-deb --build dist/ubuntu_setup
mv dist/ubuntu_setup.deb dist/RootPainter3D.deb
echo 'output debian package to dist/RootPainter3D.deb'
