...
class RPDataset(Dataset):
    def __init__(self, annot_dir, dataset_dir, in_w, out_w,
                 in_d, out_d, classes, mode, tile_refs=None, length=None):
        """
    ...
    def get_random_tile_3d(self, annot, image, fname):
        # this will find something eventually as we know
        # all annotation contain labels somewhere
        assert annot.any() # should contain some values        
        ...
        while True:
            ...
            if np.any(annot_tile_center):
                ...
                return annot_tile_center, im_tile
    ...
    def get_train_item_3d(self, tile_ref):
        ...
        image, annot, fname  = load_train_image_and_annot(self.dataset_dir, self.annot_dir)
        annot_tile, im_tile = self.get_random_tile_3d(annot, image, fname)

        # if the annotation is not big enough then pad it out. 
        if  annot_tile.shape[1:] != (self.out_d, self.out_w, self.out_w):
            annot_tile = self.get_padded_annot(fname, annot_tile)
            
        assert  annot_tile.shape[1:] == (self.out_d, self.out_w, self.out_w), (
             f" annot is {annot.shape}")
